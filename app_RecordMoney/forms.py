...
class NewRecordMoneyForm(forms.Form): 
    trading_date = forms.CharField(
        label='交易日期', 
        widget=forms.DateInput(attrs={'class':'form-control', 'placeholder':"请输入", 'type':'date'})
    )
    trading_period = forms.CharField(
        label='所属期间', 
        widget=forms.Select(attrs={'class':'form-control'}, choices=[(2020,"2020年"), (2019,"2019年")]),
        initial=2020
    )
    ...
    trader = forms.CharField(
        label='交易商家',
        widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':"请输入交易商家"}),
        max_length = 20
    )
    summary = forms.CharField(
        label = '摘要', 
        widget = forms.Textarea(attrs={'class':'form-control', 'rows':'2', 'placeholder':"请对交易事项做简要描述"}),
        max_length = 255
    )
    amount = forms.FloatField(
        label = '金额(元)',
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder':"请输入交易金额，最多保留2位小数"})
    )
